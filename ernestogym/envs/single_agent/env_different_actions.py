...
from .rewards import operational_cost, linearized_degradation
from ernestogym.ernesto.energy_storage.bess import BatteryEnergyStorageSystem
from ernestogym.ernesto import PVGenerator, EnergyDemand, EnergyMarket, DummyGenerator, DummyMarket


class MicroGridEnvDifferentAction(Env):
    """
    """
    ...
    def __init__(self,
        ...
        i_max = self._battery.get_feasible_current(self._battery.soc_min, dt=self._env_step)[0]
        i_min = self._battery.get_feasible_current(self._battery.soc_max, dt=self._env_step)[1]
        self.action_space = Box(low=i_min, high=i_max, dtype=np.float32, shape=(1,))     #CHANGED
    ...
    def reset(self, seed=None, options=None):
        ...
        # Otherwise we take an index between [1,len-1] so that we won't have out-of-index issues
            ...
            demand_idx = np.random.randint(low=1, high=len(self.demand) - 1)

        _, sampled_time, _ = self.demand[demand_idx]
    ...
    def step(self, action: np.ndarray):
        ...
        i_to_apply = np.clip(action[0], a_min=i_min, a_max=i_max)

        # to_load = np.clip(a=margin * action[0], a_min=last_v * i_min, a_max=last_v * i_max)
        to_load = last_v * i_to_apply
        # to_trade = margin - to_load
        to_trade = info_pre_step['generation'] - info_pre_step['demand'] - to_load

        # Step of the battery model and update of internal state
        self._battery.step(load=to_load, dt=self._env_step, k=self.iterations)
        self._battery.t_series.append(self.elapsed_time)
        self.elapsed_time += self._env_step
        self.iterations += 1

        # Termination condition
        terminated = bool(self._battery.soh_series[-1] <= self._termination['min_soh'])

        # Truncation conditions (due to the end of data)
        truncated = bool(
            (self._termination['max_iterations'] is not None and
             self.iterations >= self._termination['max_iterations'])
        ...
        r_clipping = abs((action[0] - i_to_apply) * last_v)
        ...
        reward = (self._trading_coeff * normalized_reward_terms[0] -
                  self._op_cost_coeff * normalized_reward_terms[1] -
                  self._deg_coeff * normalized_reward_terms[2] -
                  self._clip_action_coeff * normalized_reward_terms[3])
    ...
    def _optional_reward(self):
        ...
        if self._aging_options['degradation'] and self._deg_coeff > 0:
            deg_term = linearized_degradation(
                delta_deg=self._battery.get_delta_degradation(),
                replacement_cost=self._battery.nominal_cost,
                deg_limit=1 - self._termination['min_soh'])

        # Operational cost penalty
        if self._op_cost_coeff > 0:
            op_cost_term = (
