import Modbus_Read_Write_Float_Functions as Mb
...
class Cycler:
    ...
    # def write_modbus_multiple_holding_registers(self, start_register, val, flag):
    #     """
    #     Scrive una lista di valori float nei registri Modbus TCP.
    #     :param start_register: Registro iniziale per la scrittura
    ...
    #     #print(f"✅ Connesso al server Modbus {ip}:{port}")
    #     if flag == 0:                                       # floats 
    #         payload = hfunc.floats_to_modbus(val)
    #     elif flag == 1:
    #         payload = hfunc.u16_to_modbus(val)
    #     elif flag == 2:
    #         payload = hfunc.u32_to_modbus(val)
    ...
    def _follow_P_loop(self):
        ...
        while not self._stop_event.is_set():
            # Acquire the lock to safely read shared parameters that may be
            # modified from another thread (like when the user changes P_set).
            with self._lock:
                # If no power setpoint has been defined yet, wait and loop again.
                if self._P_set is None:
                    time.sleep(self._update_interval)
                    continue

                # Copy the shared variables locally to avoid using the lock
                # for longer than necessary (good practice for concurrency).
                P_set = self._P_set
                I_max = self._I_max
                V_min = self._V_min
                V_max = self._V_max

            # --- Begin control logic (no lock needed here) ---

            # Measure the actual voltage from the device.
            V_read = self.read_V_meas()
            print(f"\nVoltage = ({V_read:.3f} V)\n")

            # Safety check: if voltage goes outside allowed range,
            # stop the operation and warn the user, then exit the loop.
            if V_read < V_min or V_read > V_max:
                self.stop_operation()
                warnings.warn(
                    f"Voltage out of bounds ({V_read:.3f} V), operation stopped"
                )
                break

            # Compute the desired current to maintain the requested power.
            # Power = Voltage × Current → Current = Power / Voltage
            I_computed = P_set / V_read

            # Limit the current within ±I_max to protect the hardware.
            I_set = np.clip(I_computed, -I_max, I_max)

            # If clipping occurred, issue a warning for logging/debugging.
            if I_computed < -I_max or I_computed > I_max:
                warnings.warn(
                    f"I_computed = {I_computed:.3f} A was clipped to I_set = {I_set:.3f} A"
                )

            # Send the new current setpoint to the device (e.g., via Modbus).
            self.set_I_setpoint(I_set)

            # Wait for the defined interval before recalculating again.
            # This defines how fast the control loop updates the current.
            time.sleep(self._update_interval)

    # Public method to start following a new P_set
    def set_P_setpoint(self, P_set, I_max, V_min, V_max, update_interval=1.0):
        """
        ...
        # Acquire the lock to safely update shared variables that are used
        # by the background control loop (_follow_P_loop).
        with self._lock:
            self._P_set = P_set             # Desired power to maintain
            self._I_max = I_max             # Current limit for clipping
            self._V_min = V_min             # Minimum allowed voltage
            self._V_max = V_max             # Maximum allowed voltage
            self._update_interval = update_interval  # Loop update period
            

        # Check if the control thread is already running
        # (so it doesn’t accidentally start multiple loops).
        if self._thread is None or not self._thread.is_alive():
            # Reset the stop signal in case it was set previously
            self._stop_event.clear()

            # Create a new background thread that runs the control loop.
            # daemon=True -> automatically exit when the main program terminates.
            self._thread = threading.Thread(
                target=self._follow_P_loop,
                daemon=True
            )
    ...
    def stop_P_setpoint(self):
        """
        Stop the background power-control loop and safely end the operation.

        This method:
        1. Sends a stop command to the hardware (optional safety action),
        2. Signals the control thread to exit,
        3. Waits for the thread to finish,
        4. Clears the stored setpoint.
        """

        # Stop hardware activity immediately for safety
        self.stop_operation()

        # Signal the background thread to stop the control loop
        self._stop_event.set()

        # Wait for the thread to finish execution (clean shutdown)
        if self._thread is not None and self._thread.is_alive():
            self._thread.join()

        # Reset the setpoint to None to mark that control has stopped
        self._P_set = None
